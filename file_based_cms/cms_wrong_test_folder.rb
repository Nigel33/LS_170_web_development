...
def data_path
	...
	def load_file_contents(file)
		...
		yml_content = YAML.load_file(pattern(file))
...
before do 
	@files = Dir.glob(pattern("*")).map do |path|
		File.basename(path) 
	end.reject {|path| File.extname(path) == ".yml"} 

	@yml_file = Dir.glob(pattern("*.yml")).map do |path|
		File.basename(path) 
	end.join 
...
def credentials_correct?(username, password, database)
	database.key?(username) && database[username] == password
end  
...
post "/users/sign_in" do 
	...
	user_database = {}
	user_database = load_users_passwords(@yml_file)
	if credentials_correct?(username, password, user_database)
